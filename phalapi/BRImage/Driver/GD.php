...
/*
 * @Author: Brightness
 * @Date: 2022-04-02 13:56:37
 * @LastEditors: Brightness
 * @LastEditTime: 2022-04-02 13:56:37
 * @Description:  GD driver
*/

class BRImage_Driver_Gd
{
    /* 驱动相关常量定义 */
    ...
    public function open($imgname) {
        ...
        //检测图像文件
        if (!is_file($imgname)) {
            throw new PhalApi_Exception_BadRequest('不存在的图像文件', 1);
        };
        ...
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            throw new PhalApi_Exception_BadRequest('非法图像文件', 1);
        }
        ...
        if ('gif' == $this->info['type']) {
            ...
            $class = BRImage_Driver_GIF::class;
    ...
    public function save($imgname, $type = null, $quality = 80, $interlace = true) {
        ...
        if (empty($this->img)) {
            throw new PhalApi_Exception_BadRequest('没有可以被保存的图像资源', 1);
        }
    ...
    public function width() {
        ...
        if (empty($this->img)) {
            throw new PhalApi_Exception_BadRequest('没有指定图像资源', 1);
        }
    ...
    public function crop($w, $h, $x = 0, $y = 0, $width = null, $height = null) {
        ...
        if (empty($this->img)) {
            throw new PhalApi_Exception_BadRequest('没有可以被裁剪的图像资源', 1);
        }
    ...
    public function thumb($width, $height, $type = self::IMAGE_THUMB_SCALE) {
        ...
        if (empty($this->img)) {
            throw new PhalApi_Exception_BadRequest('没有可以被缩略的图像资源', 1);
        }
        ...
        switch ($type) {
            ...
            default:
                throw new PhalApi_Exception_BadRequest('不支持的缩略图裁剪类型', 1);
        }
    ...
    public function water($source, $locate = self::IMAGE_WATER_SOUTHEAST, $alpha = 80) {
        ...
        //资源检测
        if (empty($this->img)) {
            throw new PhalApi_Exception_BadRequest('没有可以被添加水印的图像资源', 1);
        }
        if (!is_file($source)) {
            throw new PhalApi_Exception_BadRequest('水印图像不存在', 1);
        }

        //获取水印图像信息
        $info = getimagesize($source);
        if (false === $info || (IMAGETYPE_GIF === $info[2] && empty($info['bits']))) {
            throw new PhalApi_Exception_BadRequest('非法水印文件', 1);
        }
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    throw new PhalApi_Exception_BadRequest('不支持的水印位置类型', 1);
                }
        }
    ...
    public function text($text, $font, $size, $color = '#00000000', $locate = self::IMAGE_WATER_SOUTHEAST, $offset = 0, $angle = 0) {
        ...
        //资源检测
        if (empty($this->img)) {
            throw new PhalApi_Exception_BadRequest('没有可以被写入文字的图像资源', 1);
        }
        if (!is_file($font)) {
            throw new PhalApi_Exception_BadRequest('不存在的字体文件：', 1);
        }
        ...
        switch ($locate) {
            ...
            default:
                ...
                if (is_array($locate)) {
                    ...
                    throw new PhalApi_Exception_BadRequest('不支持的文字位置类型', 1);
                }
        }
        ...
        if (is_string($color) && 0 === strpos($color, '#')) {
            ...
            throw new PhalApi_Exception_BadRequest('错误的颜色值', 1);
        }
