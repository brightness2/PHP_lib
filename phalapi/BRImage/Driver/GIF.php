...
/*
 * @Author: Brightness
 * @Date: 2022-04-02 14:05:45
 * @LastEditors: Brightness
 * @LastEditTime: 2022-04-02 14:05:45
 * @Description:  
*/
class BRImage_Driver_GIF
{
    /**
     * GIF帧列表
     * @var array
    ...
    public function __construct($src = null, $mod = 'url') {
        if(!is_null($src)){
            ...
            try{
                ...
                $de = new BRImage_Driver_GIFDecoder($src);
                $this->frames = $de->GIFGetFrames();
                $this->delays = $de->GIFGetDelays();
            } catch(\Exception $e){
                throw new PhalApi_Exception_BadRequest('解码GIF图片出错', 1);
            }
        }
    }
    ...
    public function save($gifname){
        ...
        $gif = new BRImage_Driver_GIFEncoder($this->frames, $this->delays, 0, 2, 0, 0, 0, 'bin');
