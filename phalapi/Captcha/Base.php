...
/*
 * @Author: Brightness
 * @Date: 2022-04-02 08:47:05
 * @LastEditors: Brightness
 * @LastEditTime: 2022-04-02 08:47:05
 * @Description:  验证码生成类
*/
...
class Captcha_Base{
    ...
    public function create( $config = [])
    {
        ...
        // file_put_contents('./testca.png',$content);
    ...
    public function check( $code)
    {
        ...
        $sessionData = $_SESSION['captcha'];
        $key = $sessionData['key'];
        $createTime = $sessionData['createTime'];

        //校验是否过期
        if($this->expire >0 && (time() - $createTime) > $this->expire){
            return false;
        }
    ...
    protected function generate()
    {
        ...
        //session 存储hash值
        $_SESSION['captcha'] = ['key'=>$hash,'createTime'=>time()];
